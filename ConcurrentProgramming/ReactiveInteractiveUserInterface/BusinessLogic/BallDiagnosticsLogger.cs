...
namespace TP.ConcurrentProgramming.BusinessLogic
{
    ...
    internal class BallDiagnosticsLogger : IBallDiagnosticsLogger
    {
        ...
        private string SerializeBallState(int ballId, double positionX, double positionY,
            double velocityX, double velocityY, double mass, double radius)
        {
            string timestamp = DateTimeOffset.Now.ToString("yyyy-MM-dd HH:mm:ss.fff", System.Globalization.CultureInfo.InvariantCulture);

            return FormattableString.Invariant($"{timestamp}|{ballId}|{positionX:F4}|{positionY:F4}|{velocityX:F4}|{velocityY:F4}|{mass:F4}|{radius:F4}");
        }
